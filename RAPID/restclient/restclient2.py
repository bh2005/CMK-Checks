...
if __name__ == '__main__':
    ...
    try:
        ...
        if args.apiKey:
            authenticateSession(args.serverRoot, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
        else:
        ...
        if args.newNodeName:
            check_database_exists(args.serverRoot, args.dbName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
            createNode(args.serverRoot, args.dbName, args.newNodeName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
            sys.exit(0)

        # Create a new item if --create-item is specified
        if args.newItemName:
            check_database_exists(args.serverRoot, args.dbName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
            node_exists = check_node_exists(args.serverRoot, args.dbName, args.nodeName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
            if node_exists:
                createItem(args.serverRoot, args.dbName, args.nodeName, args.newItemName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
            else:
        ...
        if args.target_node_status:
            ...
            if args.target_node_status.lower() == 'all':
                nodes = listNodes(args.serverRoot, args.dbName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
                if nodes:
                    for node_name in nodes:
                        status = getNodeStatus(args.serverRoot, args.dbName, node_name, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
                ...
                node_exists = check_node_exists(args.serverRoot, args.dbName, args.target_node_status, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
                if node_exists:
                    status = getNodeStatus(args.serverRoot, args.dbName, args.target_node_status, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
                    if status is not None:
                        node_status = status.get('ConnectionStatus', {}).get('ConnectionStatus')
                        connection_error = status.get('ConnectionError')
                        last_update = status.get('LastUpdate')
        ...
        if args.get_item:
            ...
            node_exists = check_node_exists(args.serverRoot, args.dbName, args.nodeName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
            if node_exists:
                found_item = getItem(args.serverRoot, args.dbName, args.nodeName, args.itemName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
                if found_item:
        ...
        found_database = check_database_exists(args.serverRoot, args.dbName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
        if not found_database:
            print("Keine Datenbank gefunden. Beende.")
            sys.exit(-1)

        # Verify if the node exists and print the result
        found_node = check_node_exists(args.serverRoot, args.dbName, args.nodeName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
        if not found_node and not args.newNodeName and not args.newItemName: # Nur prüfen, wenn kein neuer Knoten oder Item erstellt wird
            ...
            sys.exit(-1)
        elif not check_node_exists(args.serverRoot, args.dbName, args.nodeName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies) and args.newItemName:
        ...
        if not args.get_item and not args.newItemName:
            found_item = check_item_exists(args.serverRoot, args.dbName, args.nodeName, args.itemName, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
            if not found_item:
                print(f"Item '{args.itemName}' nicht gefunden. Beende.")
                sys.exit(-1)

        # Write data to the database
        # WriteProcessData(args.serverRoot, args.dbName, args.nodeName, args.itemName, ts, args.eventsToWrite, args.apiKey, verify_ssl if verify_ssl else False, verbose, proxies=proxies)
