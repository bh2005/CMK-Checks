...
# bh2005 (2.4 / 2.5)
#
# CMK 2.5: get_site_config removed -- use config.sites dict directly
...
def _my_paint_custom_notes(what: str, row: Row, config: Config) -> CellSpec:
    ...
    if what == "service":
        notes_dir = Path(default_config_dir) / "notes/services"
        dirs = match_path_entries_with_item([notes_dir], host)
        item = svc
    else:
    ...
    def replace_tags(text: str) -> str:
        sitename = row["site"]
        url_prefix = config.sites.get(sitename, {}).get("url_prefix", "/")
        return (
