...
def parse_enviromux_micro(info):
    parsed = {}
...
def parse_enviromux_micro_external(info):
    parsed = {}
...
def parse_enviromux_micro_digital(info):
    parsed = {}
...
def inventory_enviromux_micro_temperature(parsed):
    for item, sensor_data in parsed.items():
        if sensor_data["sensor_type"] in ["temperature", "temperatureCombo"]:
            yield item, {}


def inventory_enviromux_micro_voltage(parsed):
    for item, sensor_data in parsed.items():
        if sensor_data["sensor_type"] == "power":
            yield item, {}


def inventory_enviromux_micro_humidity(parsed):
    for item, sensor_data in parsed.items():
...
def enviromux_scan_function(oid):
    return oid(".1.3.6.1.2.1.1.2.0").startswith(".1.3.6.1.4.1.3699.1.1.12") or oid(".1.3.6.1.2.1.1.2.0").startswith(".1.3.6.1.4.1.3699.1.1.10") or oid(".1.3.6.1.2.1.1.2.0").startswith(".1.3.6.1.4.1.3699.1.1.9")
...
def check_enviromux_micro_temperature(item, params, parsed):
...
def check_enviromux_micro_voltage(item, params, parsed):
...
def check_enviromux_micro_humidity(item, params, parsed):
