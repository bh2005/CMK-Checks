...
def calculate_uptime(uptime_ms: Optional[int]) -> Optional[str]:
    """Wandelt die Uptime in Millisekunden in das Format 'days, hours:minutes:seconds' um."""
    if not isinstance(uptime_ms, (int, float)):
        return None

    try:
        uptime_delta = datetime.timedelta(milliseconds=int(uptime_ms))
        days = uptime_delta.days
        hours, remainder = divmod(uptime_delta.seconds, 3600)
        minutes, seconds = divmod(remainder, 60)
        return f"{days} days, {hours:02}:{minutes:02}:{seconds:02}"
    except OverflowError:
        return "Uptime zu groß für timedelta"
    except TypeError:
        return None
    ...
    while True:
        ...
        try:
            ...
            if not data:
                ...
                break
            for device in data:
                all_devices.append(process_device(device))
